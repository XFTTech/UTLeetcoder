...
const update_submission = async (user) => {
    await getRecentSubmissionList(user)
        .then(async (res) => {
            // console.log(res)
            await writeData(`../data/raw/${user}.json`,
                JSON.stringify(res[0]));
            await writeData(`../data/last_submission/${user}.json`,
...
const update_daily_log = async (user) => {
    await getDailyLogs(user)
        .then(async (res) => {
            // console.log(res)
            await writeData(`../data/daily_log/${user}.json`,
...
const split_daily_all_users = async (users) => {
    ...
    for (const date of user_all_daily.keys()) {
        await writeData(`../data/daily_stats/${date}.json`,
